...
<xsl:template match="text()">
  ...
  <xsl:choose>
    ...
    <xsl:when test="$normalize-space = '1'"><xsl:value-of select="xhtml2rtf:RTFEncode(., string(normalize-space(.)), 0)"/></xsl:when>
    <xsl:otherwise><xsl:value-of select="xhtml2rtf:RTFEncode(., string(.), $my-normalize-space)"/></xsl:otherwise>
  </xsl:choose>
</xsl:template>

  <msxsl:script language="CSharp" implements-prefix="xhtml2rtf">
    <msxsl:assembly name="Diary.Net.Functions" />
    public string CharCode(string strText)
    {
    return Diary.Net.Html2Rtf.Functions.CharCode(strText);
    }

    public string RTFEncode(XPathNodeIterator objXMLNodes,
    string strText, double intMyNormalizeSpaces)
    {
    return Diary.Net.Html2Rtf.Functions.RTFEncode(objXMLNodes,strText,intMyNormalizeSpaces);
    }

    double multiplyWidth = 192;
    double marginWidth = 196;
    double[] arrMaxLen = new double[0];
    double[] arrTotLen = new double[0];

    public string TableCellWidthFill(XPathNodeIterator objXMLNodes,
    double tableWidth)
    {
    Diary.Net.Html2Rtf.Functions.arrMaxLen = arrMaxLen;
    Diary.Net.Html2Rtf.Functions.arrTotLen = arrTotLen;
    Diary.Net.Html2Rtf.Functions.multiplyWidth = multiplyWidth;
    Diary.Net.Html2Rtf.Functions.marginWidth = marginWidth;

    string ret = Diary.Net.Html2Rtf.Functions.TableCellWidthFill(objXMLNodes,tableWidth);

    arrMaxLen = Diary.Net.Html2Rtf.Functions.arrMaxLen;
    arrTotLen = Diary.Net.Html2Rtf.Functions.arrTotLen;
    multiplyWidth = Diary.Net.Html2Rtf.Functions.multiplyWidth;
    marginWidth = Diary.Net.Html2Rtf.Functions.marginWidth;

    return ret;
    }

    public double GetTableColumnWidth(int iColumn, double bSum, double font_size)
    {
    Diary.Net.Html2Rtf.Functions.arrMaxLen = arrMaxLen;
    Diary.Net.Html2Rtf.Functions.arrTotLen = arrTotLen;
    Diary.Net.Html2Rtf.Functions.multiplyWidth = multiplyWidth;
    ...
    double ret = Diary.Net.Html2Rtf.Functions.GetTableColumnWidth(iColumn, bSum, font_size);
